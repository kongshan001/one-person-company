...
| `GET` | `/` | 首页 - 创建表单 |
| `GET` | `/health` | 健康检查 |
| `GET` | `/api/list` | 列出最近的粘贴（支持 `?q=关键词` 搜索） |
| `POST` | `/api/create` | 创建粘贴 |
| `GET` | `/raw/{id}` | 获取纯文本内容 |
| `GET` | `/{id}` | 查看粘贴（HTML 渲染） |
...
curl -X POST http://localhost:9292/api/create \
  ...
  -d '{"content": "Hello World", "title": "测试", "syntax": "text"}'
```
...
│   └── cron/                   # 定时任务
├── tests/                      # 🧪 单元测试
│   ├── test_paste_hut.py       # PasteHut 测试
│   ├── test_ping_bot.py        # PingBot 测试
│   ├── test_icon_forge.py      # IconForge 测试
